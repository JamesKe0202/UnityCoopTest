...
# 2025/12/22 IRIS v0.1 增补说明  
## 1. 总体改动概要（Summary）

- 原 v0.1 结构（抽象）：
  - Observe → Update(IRIS) → Climate / Environment → Render
- 新结构：引入 **用户情感画像** 并采用 **双通道并行（Parallel Tracks）**：
  - Track A（慢通道）：Observe → Infer(UserAffectProfile)
  - Track B（快通道）：Observe → Update(WorldState) → Render(Content)
- 时间基准：
  - 玩家输入仍按 **实时事件（Player Clock）** 触发
  - WorldState 与 UserAffectProfile 的演化改为以 **游戏内时间单位 IrisDay** 为主时间轴（World Clock）

一句话总结：

- One event stream, two consumers, different clocks (PlayerClock & IrisDayClock).

---

## 2. 数据对象定义（Data Contracts）

### 2.1 Observe 输出：UserSignals（统一事件总线）

Observe 层负责采集玩家行为，并统一包装为 UserSignalEvent，作为系统内部的 Signal Bus。

    [System.Serializable]
    public class UserSignalEvent
    {
        public long   timestamp;        // 可选：真实时间戳
        public string eventType;        // e.g. "Move", "NpcDialogueChoice", "QuestDecision"
        public string sceneId;          // 场景 / 区域 ID
        public string actorId;          // 相关 NPC / 物体 ID（可选）
        public string extraPayload;     // JSON，扩展特征（可选）

        // 归一化特征（0..1），按需要填充
        public float decisionTime01;      // 决策耗时
        public float riskLevel01;         // 行为风险程度
        public float socialProximity01;   // 社交接近度
    }

说明：

- UserSignalEvent 是 **唯一的 Observe 输出格式**
- Track A / Track B **并行订阅** 此事件流
- Observe 不进行推断、不进行决策，仅负责“采集 + 派发”

---

### 2.2 Track A 输出：UserAffectProfile（用户情感画像，慢变量）

UserAffectProfile 描述玩家较为稳定的情感/认知取向，更新频率低，带置信度。

    [System.Serializable]
    public class TraitValue
    {
        public float value;        // 0..1 当前估计
        public float confidence;   // 0..1 置信度
    }

    [System.Serializable]
    public class UserAffectProfile
    {
        // 维度名称仅为示例，可按设计调整
        public TraitValue meaningSource;        // 外部确认 vs 内部一致性
        public TraitValue uncertaintyStrategy;  // 结构控制 vs 即兴适应
        public TraitValue emotionalBoundary;    // 情绪渗透 vs 情绪边界清晰
        public TraitValue agencyAttribution;    // 自我主导 vs 世界主导

        public float lastUpdatedIrisDay;        // 上次更新时的 irisDay（0..N）
    }

工程约定：

- 画像由 ProfileService 维护，存放于 ProfileStore
- Track B 通过 ProfileStore.GetLatestSnapshot() 读取最新画像（非阻塞、允许为 null）

---

### 2.3 （可选）UserMoodState（短期状态，快变量）

UserMoodState 表达“这一小段时间内”的紧张/疲劳/好奇程度，可结合实时事件与 IrisDay 同步。

    [System.Serializable]
    public class UserMoodState
    {
        public float tension;      // 紧张度 0..1
        public float fatigue;      // 疲劳度 0..1
        public float curiosity;    // 好奇度 0..1

        public float lastUpdatedIrisDay;
    }

用途：

- Track B 用于即时体验稳定（例如 tension 很高时减少刺激事件密度）
- 不属于长期画像；缺失时不影响主逻辑

---

## 3. 双通道结构（Parallel Tracks）

### 3.1 总体结构示意

    Observe(UserSignals)
        ├── Track A（慢通道）: Infer UserAffectProfile（用户画像推断）
        └── Track B（快通道）: Update WorldState → Render Content

说明：

- 同一份 UserSignals 被两个通道消费
- Track A 和 Track B 运行在不同“节奏”（不同 clock）
- Track B 不依赖 Track A 的即时输出；Profile 只作为可选偏置（bias）

---

## 4. Track A：用户画像推断（Slow Loop）

### 4.1 职责

- 输入：一定 IrisDay 时间窗口内的 UserSignalEvent 列表（如过去 0.25 IrisDay）
- 输出：更新后的 UserAffectProfile（以及可选 UserMoodState）
- 不参与逐帧渲染，不直接修改 WorldState

### 4.2 更新节奏（基于 IrisDay）

示例设定：

- irisDay ∈ [0, 1) 表示一天完整的游戏内时间  
- 每帧推进 irisDay += deltaIris；到 1 后归零或继续累积

推荐策略：

- 每经过 ΔIrisDay_Profile（例如 0.25）或关键叙事节点时调用一次 UpdateUserProfile

### 4.3 伪代码示意

    void UpdateUserProfile(
        UserAffectProfile profile,
        List<UserSignalEvent> recentSignalsWindow,
        float currentIrisDay
    )
    {
        // 1. 聚合 recentSignalsWindow → 行为统计特征
        // 2. 使用规则 / EMA / 贝叶斯 / ML 模型更新各 TraitValue.value 与 confidence
        // 3. 保存快照
        profile.lastUpdatedIrisDay = currentIrisDay;
        ProfileStore.Save(profile);
    }

---

## 5. Track B：世界更新与渲染（Fast Loop）

### 5.1 职责

- 输入：实时 UserSignals + 当前 irisDay
- 更新：WorldState（包含 iris 指标、环境、BGM、Zone 等）
- 输出：供引擎侧渲染的具体环境参数、BGM 模式、空间解锁等
- 不等待画像更新；Profile 仅作为可选偏置层

### 5.2 事件驱动入口（PlayerClock）

    void OnUserSignal(UserSignalEvent e)
    {
        // 1. 将事件写入短期缓冲（用于即时反应或统计）
        // 2. 根据事件类型决定是否立即触发一次世界更新
        //    如：对话选择、战斗结果、进入新区域等
    }

### 5.3 基于 IrisDay 的周期性世界更新（WorldClock）

在主循环中，通过 irisDay 进行世界惯性与环境演化。

    float irisDay;                 // 0..1 或累积值
    float lastWorldUpdateIrisDay;  // 上次世界更新的 irisDay
    float lastProfileUpdateIrisDay;// 上次画像更新的 irisDay

    void Tick(float dt)
    {
        // 推进游戏内时间
        irisDay += dt * irisDaySpeed;
        if (irisDay >= 1f) irisDay -= 1f; // 或不归零，视设计而定

        // WorldState：每 ΔIrisDay_World 更新一次，例如 0.01
        if (irisDay - lastWorldUpdateIrisDay >= 0.01f)
        {
            lastWorldUpdateIrisDay = irisDay;
            var profileSnapshot = ProfileStore.GetLatestSnapshot();
            var moodSnapshot     = MoodStore.GetLatestSnapshot();
            UpdateWorld(worldState, irisDay, profileSnapshot, moodSnapshot);
        }

        // User Profile：每 ΔIrisDay_Profile 更新一次，例如 0.25
        if (irisDay - lastProfileUpdateIrisDay >= 0.25f)
        {
            lastProfileUpdateIrisDay = irisDay;
            var recentWindow = SignalBuffer.CollectWindow(irisDay, 0.25f);
            UpdateUserProfile(userProfile, recentWindow, irisDay);
        }
    }
...
在原有 v0.1 的 UpdateWorld 基础上，加入对 UserAffectProfile / UserMoodState 的可选读取。

    void UpdateWorld(
        WorldState       world,
        float            irisDay,
        UserAffectProfile profileSnapshot, // 可为 null
        UserMoodState     moodSnapshot     // 可为 null
    )
    {
        // 0) 可选：根据短期 mood 做即时稳定
        //    例如：tension 很高 → 降低事件密度或减少视觉噪声

        // 1) v0.1：根据任务、面板、事件更新 world.iris
        //    IrisPanelUpdater.Apply(...)
        //    IrisEventApplier.Apply(...)
        //    world.iris.Clamp01();

        // 2) v0.1：根据 iris 选择 ClimateArchetype
        world.env.climateArchetype =
            ClimateSelector.SelectArchetype(world.iris);

        // 3) v0.1：根据 ClimateArchetype 应用环境预设
        EnvironmentPresets.ApplyClimatePreset(world.env);

        // 4) v0.1：结合 iris 与 irisDay 对环境参数进行细调
        EnvironmentAdjuster.AdjustEnvironment(
            world.env,
            world.iris,
            irisDay
        );

        // 4b) 新增：若存在画像快照，对环境参数做小幅偏置（small bias）
        if (profileSnapshot != null)
        {
            ApplyProfileBiasToEnvironment(world.env, profileSnapshot);
        }

        // 5) v0.1：选择 BGM 模式
        world.sound.currentBgmMode =
            BgmSelector.SelectBgm(world.iris, world.env.climateArchetype);

        // 6) v0.1：更新特殊空间 / 区域解锁状态
        ZoneUnlocker.UpdateOpenZones(world, /* questState */ null);
    }
...
画像偏置是一个附加层，不改变 v0.1 既有规则，仅在 preset 的基础上做轻微倾向性调整。

    void ApplyProfileBiasToEnvironment(
        EnvironmentState  env,
        UserAffectProfile profile
    )
    {
        // 示例1：偏好“结构控制”的玩家 → 世界略微更有秩序感
        if (profile.uncertaintyStrategy.value > 0.7f)
        {
            env.flowOrderliness = Mathf.Clamp01(env.flowOrderliness + 0.1f);
            env.npcPathNoise    = Mathf.Clamp01(env.npcPathNoise - 0.1f);
        }

        // 示例2：更依赖“外部确认”的玩家 → 城市中反馈信号略微增多
        if (profile.meaningSource.value > 0.7f)
        {
            env.publicAlertLevel = Mathf.Clamp01(env.publicAlertLevel + 0.1f);
            env.adIntensity      = Mathf.Clamp01(env.adIntensity + 0.05f);
        }
    }

设计原则：

- 仅做 Small Bias，不覆盖 ClimateSelector / EnvironmentPresets / EnvironmentAdjuster 的主导逻辑
- 当 profileSnapshot == null 时，UpdateWorld 的行为应与 v0.1 完全一致，保证兼容性

---

## 8. 更新频率与时间基准（Two Clocks + IrisDay）

### 8.1 两套时钟

- PlayerClock：由玩家输入和引擎事件驱动（实时）
- WorldClock：由游戏内时间 irisDay 驱动

### 8.2 各模块的频率关系

| 模块                         | 时钟来源      | 建议频率                                       |
|------------------------------|---------------|-----------------------------------------------|
| Observe（UserSignals）       | PlayerClock   | 每个关键事件即时写入                           |
| Track B：World Update        | WorldClock    | 每 ΔIrisDay_World 更新一次（如 0.01）          |
| Track A：User Profile        | WorldClock    | 每 ΔIrisDay_Profile 更新一次（如 0.25）或关键节点触发 |

一句话总结：

- Profile is slow; world feel is per IrisDay.

---

## 9. LLM 接入位置（如后续使用）

- LLM 仅位于 Render(Content) 层，用于：
  - 生成 NPC 对话文本
  - 生成系统旁白 / 城市广播 / 日记等叙事内容
- LLM 输入上下文可包括：
  - 当前 WorldState（气候、社会氛围等）
  - 当前 UserAffectProfile（作为文本风格与倾向的 bias）
- LLM 不得：
  - 直接修改 UserAffectProfile
  - 直接修改 WorldState 数值或 IrisState

通过此约束，可以保证：

- 核心状态机（Profile + WorldState）可测试、可重放、可重现  
- LLM 仅负责可替换的表现层，不影响核心逻辑稳定性

---

## 10. 最小侵入式改造 Checklist

1. 新增 UserSignalEvent 结构与事件总线（Signal Bus）  
2. 新增 ProfileService / ProfileStore，实现 Track A（画像慢通道）  
3. 在世界更新流程中增加可选参数：UserAffectProfile profileSnapshot, UserMoodState moodSnapshot  
4. 在 UpdateWorld 中调用 ApplyProfileBiasToEnvironment（可选，小幅偏置）  
5. 在主循环 Tick 中维护 irisDay、lastWorldUpdateIrisDay、lastProfileUpdateIrisDay 等变量  
6. （可选）新增 UserMoodState 及其更新逻辑  
7. 保留 v0.1 中 IrisState / WorldState / EnvironmentState 及原有系统，实现向后兼容

---

## 11. 预期效果（Behavior Summary）

- 世界仍由 IRIS v0.1 的规则与状态主导  
- 玩家行为通过 Track B 获得 **即时环境反馈**（光线、雾、BGM、事件密度等）  
- 玩家在更长时间尺度上的行为模式被 Track A 累积为 **用户情感画像**，对世界整体气质产生缓慢而可感的影响  
- 双通道 + IrisDay 的时间结构保证：
  - 实时体验流畅（Fast Loop 不被阻塞）
  - 长期适配可解释（Slow Loop 独立可调试）
  - 系统一致、可控、易于迭代与扩展
